...
def create_app(test_config=None):
  ...
  def get_questions():
    ...
    question_query = Question.query.paginate(page,QUESTIONS_PER_PAGE,False)
    question_items = [question.format() for question in question_query.items]
  ...
  def question_search():
    ...
    questions_page = Question.query.filter(Question.question.ilike(query)).paginate(page,QUESTIONS_PER_PAGE,False)
  ...
  def get_questions_by_category(id):
    ...
    question_by_category = Question.query.filter(Question.category == id).all()
    question_by_category_formatted = [question.format() for question in question_by_category]
  ...
  def get_questions_for_quiz():
    ...
    question_filter_query = Question.query.filter(Question.category == quiz_category_id)

    if quiz_category_id == 0:
      question_filter_query = Question.query
    
    questions = question_filter_query.filter(Question.id.notin_(previous_questions)).all()
